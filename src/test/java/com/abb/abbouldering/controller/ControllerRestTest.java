...
class ControllerRestTest {
	...
	void init() throws Exception {
		...
		admin1 = new UserBuilder().email("admin1@asdf.com").password("Password123").role(Role.ADMIN).firstName("first")
				.lastName("last").build();
		admin2 = new UserBuilder().email("email2@email.com").password("Password123").role(Role.ADMIN).firstName("first")
				.lastName("last").build();
		user = new UserBuilder().email("newUser3@email.com").password("Password123").role(Role.USER).firstName("first")
				.lastName("last").build();
		...
		event1 = new Event("Event1", "smallDescription1", "description1", 0.0, 10, LocalDateTime.now(), admin1, "imageUrl1");
		event2 = new Event("Event2", "smallDescription2", "description2", 1.0, 10, LocalDateTime.now(), admin1, "imageUrl2");
		event3 = new Event("Event3", "smallDescription3", "description3", 10.0, 10, LocalDateTime.now(), admin2, "imageUrl3");
		event4 = new Event("Event4", "smallDescription4", "description4", 21.0, 10, LocalDateTime.now(), admin2, "imageUrl4");
		event5 = new Event("Event5", "smallDescription5", "description5", 30.0, 10, LocalDateTime.now(), admin1, "imageUrl5");
	...
	void testAuthController_register_returns201ResponseAndJwt() {
		...
		ResponseEntity<AuthenticationResponse> response = rest.postForEntity(baseUrl + "/api/v1/auth/register", request, AuthenticationResponse.class);
	...
	void testAuthController_login_returns200ResponseAndJwt() {
		...
		ResponseEntity<AuthenticationResponse> response = rest.postForEntity(baseUrl + "/api/v1/auth/authenticate", request, AuthenticationResponse.class);
	...
	void testAuthController_login_invalidCredentialsReturns403Response() {
		AuthenticationRequest request = new AuthenticationRequest("email123.com", "Password123");
		try {
